...
def main():
    ...
    # --- Reflection 2: Analyze the number of parks found ---
    original_parks = parks_data["data"]
    park_count = len(original_parks)
    print(f"Found {park_count} parks in {state}.")

    if park_count > 10:
        print("That's a lot of parks! Asking the model to select a diverse few...")
        
        park_names = [park['fullName'] for park in original_parks]
        
        whittle_down_system_prompt = "You are an expert travel guide. From the following list of national parks, select a diverse and interesting subset of up to 8 parks. Consider geographical diversity and variety in park types if possible. Return only a comma-separated list of the park names you select."
        
        whittle_down_prompt = f"Here is the list of parks: {', '.join(park_names)}"
        
        selected_park_names_str, _ = query_model(whittle_down_system_prompt, whittle_down_prompt)
        
        selected_park_names = [name.strip() for name in selected_park_names_str.split(',')]
        
        # Filter the original list to only include the selected parks
        parks_to_process = [park for park in original_parks if park['fullName'] in selected_park_names]
        print(f"Model selected these parks to focus on: {', '.join(selected_park_names)}")

    elif park_count <= 3:
        print("Found only a few parks. Proceeding with the full list.")
        parks_to_process = original_parks
    else:
        parks_to_process = original_parks
